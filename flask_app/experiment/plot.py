...
def plot_culture(culture, limit=100000):
    ...
    # stress_increase_delay_generations = culture.parameters[
    ...
    if len(concs.values()) > 0:
        next_rescue_dilution_time = max(ods.keys()) + timedelta(
        ...
        lines.append(
            go.layout.Shape(
                type="line",
                xref="x",
                yref="y3",  # Use 'y3' to align with concs trace
                x0=list(concs.keys())[-1],
                y0=list(concs.values())[-1],
                x1=next_rescue_dilution_time,
                y1=list(concs.values())[-1],
                line=dict(
                    color="green",
                    width=1,
                    dash="dot",
        ...
        lines.append(
            go.layout.Shape(
                ...
                xref="x",
                yref="y3",  # Use 'y3' to align with concs trace
                x0=next_rescue_dilution_time,
                y0=list(concs.values())[-1],
                x1=next_rescue_dilution_time,
                y1=list(concs.values())[-1] / dilution_factor,
                line=dict(
                    color="green",
                    width=1,
                    dash="dot",
    ...
    lines.append(
        go.layout.Shape(
            type="line",
            xref="x",
            yref="y1",  # Use 'y1' to align with ods trace
            x0=latest_od_x,
            y0=od_threshold,
            x1=latest_od_x + timedelta(hours=12),
            y1=od_threshold,
            line=dict(
    ...
    layout = go.Layout(
        ...
        yaxis=dict(title="Optical Density", automargin=True),
        yaxis2=dict(
            title="Generation",
            overlaying="y",
            side="right",
