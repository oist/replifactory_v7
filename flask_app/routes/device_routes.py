...
@device_routes.route(
    "/api/stock_adjust?pump_index=<int:pump_index>?volume=<int:volume>",
    methods=["POST"],
)
@device_routes.route(
    "/stock_adjust?pump_index=<int:pump_index>?volume=<int:volume>", methods=["POST"]
)
...
def set_part_calibration(devicePart):
    ...
    if devicePart == "ods":
        response = jsonify(
            ...
            coefs=dev.device_data["ods"]["calibration_coefs"][partIndex],
        )
    return response
...
def start_pump_calibration_sequence():
    ...
    print(
        f"Starting pump {pumpId} calibration sequence with {rotations} rotations and {iterations} iterations"
    )
