...
def test_is_time_to_rescue():
    ...
    assert not c.is_time_to_rescue(
        verbose=True
    ), "Expected False when last_dilution_time is None"
    ...
    assert not c.is_time_to_rescue(
        verbose=True
    ), "Expected False when growth_rate is None"
    ...
    assert not c.is_time_to_rescue(
        verbose=True
    ), "Expected False when last dilution is too recent"
    ...
    assert not c.is_time_to_rescue(
        verbose=True
    ), "Expected False when latest_t_doubling is positive and below threshold"
    ...
    assert c.is_time_to_rescue(
        verbose=True
    ), "Expected True when latest_t_doubling is negative or above threshold"
...
def object_to_dict(obj):
    ...
    for key, value in obj.__dict__.items():
        if isinstance(value, dict):
            ...
            value = repr(
                value
            )  # Otherwise, we use repr to get a string representation of the attribute value
        result[key] = value
    return result
