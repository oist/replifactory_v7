<template>
    <CCard style="width: 14rem" class="m-2 thermometer-control">
        <CCardBody>
            ...
            <CCardSubtitle class="mb-2 text-muted">{{ label }}</CCardSubtitle>
            <CInputGroup class="mb-3">
                <CButton type="button" color="success" @click="handleRefreshClick" variant="outline"
                    :class="state == 'WORKING' ? 'active' : ''" :disabled="disabled">
                    <CIcon :icon="icon.cilLoopCircular" size="xl" />
                </CButton>
                <CFormInput v-model="value" :id="deviceId + '-value'" placeholder="?" :aria-label="label + ' themperature'"
                    :aria-describedby="deviceId + '-inputLabel'" readonly />
                <CInputGroupText :id="deviceId + '-inputLabel'">&deg;C</CInputGroupText>
            </CInputGroup>
        </CCardBody>
    </CCard>
</template>
...
<script>
import { CCard, CCardBody, CInputGroup, CButton, CFormInput, CInputGroupText, CCardTitle, CCardSubtitle } from '@coreui/vue'
