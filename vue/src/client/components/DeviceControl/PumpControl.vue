<template>
  <div class="pump-controls">
    <div v-for="i in [1, 2, 4]" :key="i" class="pump">
      <button
        class="pump-button"
      ...
      <div class="pump-input">
        <div class="form-group">
          <CFormInput
            ...
            :value="volume[i]"
            :disabled="disabled"
            @input="(event) => onVolumeInput(event, i)"
          />
        </div>
        ...
        <div v-if="calibrationModeEnabled" class="form-group">
          <CFormInput
            ...
            :value="rotations[i]"
            :disabled="disabled"
            @input="(event) => onRotationsInput(event, i)"
          />
          <!--          <label for="calibration">Calibration:</label>-->
          <PumpCalibration :pump-id="i" :disabled="disabled" />
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    onVolumeInput(event, pumpId) {
      ...
      const volume = parseFloat(this.volume[pumpId]);
      if (!isNaN(volume)) {
        this.rotations[pumpId] = this.calculateRotations(
          volume,
          pumpId,
        ).toFixed(2);
      } else {
    ...
    calculateRotations(volume, pumpId) {
      ...
      const points = Object.entries(pumpCoefficients)
        .map(([rot, coef]) => [parseInt(rot), coef])
        .sort((a, b) => a[0] - b[0]);
      ...
      for (let i = 0; i < points.length - 1; i++) {
        if (
          ...
          volume <= points[i + 1][0] * points[i + 1][1]
        ) {
          lowerPoint = points[i];
      ...
      const interpolatedCoefficient =
        lowerPoint[1] + (upperPoint[1] - lowerPoint[1]) * factor;
    ...
    calculateVolume(rotations, pumpId) {
      ...
      // Calculate the interpolation factor
      const factor =
        (rotations - lowerPoint[0]) / (upperPoint[0] - lowerPoint[0]);
...
.pump input {
  ...
  text-align: center;
  margin-top: 10px;
  /* Adds a little space between the button and the input */
}
</style>
