  ...
  actions: {
    setAllStirrersStateAction({ dispatch, state }, newState) {
      Object.keys(state.stirrers.states).forEach((stirrerIndex) => {
        ...
        dispatch("setPartStateAction", {
          devicePart: "stirrers",
          partIndex: parseInt(stirrerIndex),
          newState: newState,
        });
      });
    },
    async updateODCalibrationKeyAction({ dispatch, commit, state }, payload) {
      ...
      for (let i = 1; i <= 7; i++) {
        await dispatch("setPartCalibrationAction", {
          devicePart: "ods",
          partIndex: i,
    ...
    setPartCalibrationAction({ commit }, payload) {
      ...
      for (let key in newCalibration) {
        if (newCalibration[key] === null) delete newCalibration[key];
      }

      return new Promise((resolve, reject) => {
        api
          .post(endpoint, { partIndex, newCalibration })
          .then((response) => {
            if (response.data.success) {
              ...
              commit("setPartCalibration", {
                devicePart,
                partIndex,
                newCalibration: response.data.newCalibration,
                coefs: response.data.coefs,
              });
    ...
    measureODCalibrationAction({ dispatch }, payload) {
      ...
      return new Promise((resolve, reject) => {
        api
          .post(endpoint, { odValue: parseFloat(odValue) })
          .then((response) => {
            if (response.data.success) {
    ...
    startPumpCalibrationSequence({ dispatch }, payload) {
      ...
      return new Promise((resolve, reject) => {
        api
          .post(endpoint, { pumpId, rotations, iterations })
          .then((response) => {
            if (response.data.success) {
              dispatch("setPartStateAction", {
    ...
    setPartStateAction({ commit }, payload) {
      ...
      return new Promise((resolve, reject) => {
        api
          .post(`${endpoint}`, { partIndex, newState, input })
          .then((response) => {
            if (response.data.success) {
              resolve(response.data);
            } else {
